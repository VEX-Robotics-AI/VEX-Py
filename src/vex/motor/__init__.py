...
class Motor(Device):
    ...
    def __init__(self, index: Ports, reverse: bool = False, /):
        ...
        self.velocities: dict[VelocityUnits, float] = dict[
            VelocityUnits, float
        ]()  # noqa: E501
        self.stopping: Optional[BrakeType] = None
        self.rotations: dict[RotationUnits, float] = dict[
            RotationUnits, float
        ]()  # noqa: E501
    ...
    def __eq__(self, other: Self) -> bool:
        ...
        return (
            ...
            and (other.reverse == self.reverse)
        )
    ...
    ) -> Tuple[float, VelocityUnits]:
        velocityIsNum = (type(velocity) == float) or (type(velocity) == int)
        if (velocity is None) or (not velocityIsNum):
            # Vex Py API v2
            if (self.selected_velocity_unit is None) or (
                self.selected_velocity_unit not in self.velocities
            ):
                raise ValueError(
                    "You have not selected any velocity. Please call set_velocity(velocity, velocityUnits) first."
                )
            velocity = self.velocities[self.selected_velocity_unit]
            velocityUnits = self.selected_velocity_unit
    ...
    def set_velocity(
        self, velocity: float, velocityUnits: VelocityUnits = VelocityUnits.PCT, /
    ):
        """Set Velocity."""
    ...
    @act
    def set_rotation(
        self, value: float, rotationUnits: RotationUnits = RotationUnits.DEG, /
    ):
    ...
    @act
    def set_timeout(
        self, time: float, timeUnits: TimeUnits = TimeUnits.SEC, /
    ):  # noqa: E501
    ...
    @vexcode_doc(
        ...
        Must call set_velocity(velocity, velocityUnits) before calling this method.
    ...
    def spin(
        ...
        """Spin Motor."""
        velocity, velocityUnits = self._get_selected_velocity_and_unit(
    ...
    @act
    def set_max_torque(
        self, value: float, torqueUnits: TorqueUnits = TorqueUnits.NM, /
    ):
        """Set Max Torque."""
    ...
    @sense
    def rotation(
        self, rotationUnits: RotationUnits = RotationUnits.DEG, /
    ) -> float:  # noqa: E501
    ...
    @sense
    def velocity(
        self, velocityUnits: VelocityUnits = VelocityUnits.PCT, /
    ) -> float:  # noqa: E501
